...
distances_vdw = np.array(df['tot_vdw'].dropna().index)
distances_tot = np.array(df['tot'].dropna().index)
distances_mp2 = np.array(df['_mp2'].dropna().index)
energies_vdw = np.asarray(df['tot_vdw'].dropna()) * conv
energies_tot = np.asarray(df['tot'].dropna()) * conv
energies_mp2 = np.asarray(df['_mp2'].dropna()) * conv

# Lennard-Jones fit
powers = [-12, -6]
x_vdw = np.power(distances_vdw.reshape(-1, 1), powers)
x_tot = np.power(distances_tot.reshape(-1, 1), powers)
x_mp2 = np.power(distances_mp2.reshape(-1, 1), powers)
coeffs_vdw = np.linalg.lstsq(x_vdw, energies_vdw)[0]
coeffs_tot = np.linalg.lstsq(x_tot, energies_tot)[0]
coeffs_mp2 = np.linalg.lstsq(x_mp2, energies_mp2)[0]
fpoints_vdw = np.linspace(distances_vdw.min(), distances_vdw.max(), 100).reshape(-1, 1)
fpoints_tot = np.linspace(distances_tot.min(), distances_tot.max(), 100).reshape(-1, 1)
fpoints_mp2 = np.linspace(distances_mp2.min(), distances_mp2.max(), 100).reshape(-1, 1)
fdata_vdw = np.power(fpoints_vdw, powers)
fdata_tot = np.power(fpoints_tot, powers)
fdata_mp2 = np.power(fpoints_mp2, powers)
fit_energies_vdw = np.dot(fdata_vdw, coeffs_vdw)
fit_energies_tot = np.dot(fdata_tot, coeffs_tot)
fit_energies_mp2 = np.dot(fdata_mp2, coeffs_mp2)
...
ax.plot(distances_vdw, energies_vdw, label='exchange + dispersion', marker='*', color='blue', linestyle='')
ax.plot(distances_tot, energies_tot, label=r'total interaction ($E_{\mathrm{SAPT0}}$)', marker='o', color='orange', linestyle='')
ax.plot(distances_mp2, energies_mp2, label=r'total interaction ($\Delta E_{\mathrm{MP2}}$)', marker='^', color='red', linestyle='')
ax.plot(fpoints_vdw, fit_energies_vdw, marker='', color='blue', linestyle='-', label='Lennard-Jones fit (exchange + dispersion)')
ax.plot(fpoints_tot, fit_energies_tot, marker='', color='orange', linestyle='--', label='Lennard-Jones fit (total SAPT0 interaction)')
ax.plot(fpoints_mp2, fit_energies_mp2, marker='', color='red', linestyle='-.', label='Lennard-Jones fit (total MP2 interaction)')
